...
This guide explains how to install **Caddy, Dex, Grist, MinIO, n8n, Redis** as **rootless Podman Quadlets** on a freshly installed **Debian 13** server.

It assumes:

* You have **sudo** access.
* The server is on a **LAN**: no public internet exposure required, but it does need to pull images at install time.
* You will run everything as a **dedicated non-root user**.
...
Pick a local domain you control on your LAN, for example:
 ...
 or `ss.dipvvf.it` (so you will use `apps.ss.dipvvf.it`, `grist.ss.dipvvf.it`, etc.) 
...
## Create the dedicated user for rootless Podman
...
Log in as that user or:

```bash
su - apps
```

## Install requirements (system packages)
...
Linux Fedora 43 has everything already installed.
...
## Clone the `apps-deployment` repository into the apps user home
...
## Configure secrets (especially `DOMAIN`)
...
(Use your chosen domain: `local`, `ss.dipvvf.it`, etc.)
